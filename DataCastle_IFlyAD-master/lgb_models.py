...
def lgb_model(train_data, test_data, parms, n_folds=5):
    ...
    predictions.to_csv(path + "predict.csv", index = False)
...
if __name__ == "__main__":
    ...
    train_df = get_data(name="train", stacking=False,filter_=True, qcut=True)
    test_df = get_data(name="test", stacking=False,filter_=True, qcut=True)
    print('get data done.')
